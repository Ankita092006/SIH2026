...
export default function Home() {
  ...
  useEffect(() => {
    ...
    setPendingReminders(allReminders.filter(r => r.status === 'pending' || r.status === 'upcoming').slice(0, 2));
  }, []);
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem', marginBottom: '2rem', marginTop: '1rem' }}>
        ...
        <div>
          <h1 style={{ fontSize: '2.5rem', margin: 0 }}>Hello, {profile.name.split(' ')[0]}</h1>
          <p style={{ fontSize: '1.2rem', margin: '0.5rem 0 0 0', color: 'var(--nav-text)' }}>How are you feeling today?</p>
        </div>
      </div>

      <div className="card" style={{ borderLeft: '8px solid var(--primary-color)' }}>
        <h2 style={{ fontSize: '1.8rem', marginTop: 0 }}>Today's Recommended Activity</h2>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '1.5rem' }}>
        ...
        <div className="card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '2px solid var(--secondary-color)', paddingBottom: '1rem', marginBottom: '1rem' }}>
            <h2 style={{ fontSize: '1.5rem', margin: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Bell /> Reminders</h2>
          ...
          <ul style={{ listStyle: 'none', padding: 0, fontSize: '1.2rem', textAlign: 'left', margin: 0 }}>
        ...
        <div className="card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '2px solid var(--secondary-color)', paddingBottom: '1rem', marginBottom: '1rem' }}>
            <h2 style={{ fontSize: '1.5rem', margin: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}><Trophy /> Recent Results</h2>
