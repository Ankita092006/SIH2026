  ...
  return (
    <div className="container" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', padding: '1rem' }}>
      <div className="card" style={{ width: '100%', maxWidth: '450px', margin: '0 auto', textAlign: 'center', padding: '2.5rem 2rem' }}>
        ...
        <h1 style={{ fontSize: '2.5rem', marginBottom: '0.5rem', color: 'var(--primary-color)' }}>{t('login.welcome')}</h1>
        <p style={{ marginBottom: '2rem', fontSize: '1.1rem', color: 'var(--nav-text)' }}>{t('login.enterDetails')}</p>
        ...
        <div style={{ backgroundColor: '#e2e8f0', color: '#4a5568', padding: '0.75rem', borderRadius: '4px', marginBottom: '1.5rem', fontSize: '1rem', textAlign: 'center' }}>
          <strong>{t('login.demoLogin')}</strong> PAT001 / 1234
        </div>

        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
          <div style={{ textAlign: 'left' }}>
            <label htmlFor="patientId" style={{ display: 'block', fontWeight: 'bold', marginBottom: '0.5rem', fontSize: '1.1rem' }}>{t('login.patientId')}</label>
            <input
              type="text"
              id="patientId"
              value={patientId}
              onChange={(e) => setPatientId(e.target.value)}
              placeholder={t('login.egPat001')}
              style={{ width: '100%', padding: '1rem', fontSize: '1.1rem', borderRadius: '8px', border: '2px solid var(--secondary-color)', boxSizing: 'border-box' }}
            />
          </div>
          <div style={{ textAlign: 'left' }}>
            <label htmlFor="password" style={{ display: 'block', fontWeight: 'bold', marginBottom: '0.5rem', fontSize: '1.1rem' }}>{t('login.password')}</label>
            ...
            <div style={{ textAlign: 'right', marginTop: '0.5rem' }}>
              <a href="#" style={{ fontSize: '0.9rem', color: 'var(--nav-text)', textDecoration: 'none', fontWeight: 'normal' }}>{t('login.forgotPassword')}</a>
            </div>
          </div>
          <button type="submit" style={{ marginTop: '0.5rem', width: '100%', padding: '1rem', fontSize: '1.2rem', borderRadius: '8px' }}>{t('login.loginButton')}</button>
        </form>
        ...
        {import.meta.env.VITE_GOOGLE_CLIENT_ID ? (
          <ErrorBoundary fallback={
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <button type="button" onClick={() => setError(t('login.errorGoogleLoad'))} className="btn-social" style={{ position: 'relative', padding: '1rem', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <div style={{ position: 'absolute', left: '1rem', display: 'flex', alignItems: 'center' }}>
                  <svg width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
                    <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <button type="button" onClick={() => setError(t('login.errorGoogleConfig'))} className="btn-social" style={{ position: 'relative', padding: '1rem', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <div style={{ position: 'absolute', left: '1rem', display: 'flex', alignItems: 'center' }}>
                <svg width="24" height="24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                  <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4"/>
                  <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/>
        ...
        <div style={{ marginTop: '2.5rem', fontSize: '1rem', color: 'var(--nav-text)' }}>
          {t('login.newToMindCare')} <a href="#" style={{ color: 'var(--primary-color)' }}>{t('login.signUp')}</a>
        </div>
      </div>
    </div>
  );
