...
function MemoryMatch({ onComplete, soundEnabled }) {
  ...
  useEffect(() => {
    if (cards.length > 0 && cards.every(c => c.isMatched)) {
      ...
      setTimeout(() => onComplete(Math.max(10, 100 - (turns * 2)), accuracy, `${Math.floor(timeTakenMs/1000)}s`, matches, turns - matches), 1000);
    }
  ...
  return (
    <div>
      <h2 style={{ textAlign: 'center', marginBottom: '1rem' }}>Attempts: {turns}</h2>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', maxWidth: '400px', margin: '0 auto' }}>
        {cards.map((c, i) => (
          <button key={c.id} onClick={() => handleCardClick(i)} style={{ height: '100px', fontSize: '3rem', margin: 0, backgroundColor: c.isFlipped || c.isMatched ? 'white' : 'var(--primary-color)', border: '2px solid ' + (c.isFlipped || c.isMatched ? '#ccc' : 'var(--primary-color)') }}>
...
function NumberRecall({ onComplete, soundEnabled, triggerFeedback }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (round < maxRounds) {
      ...
      setTimeout(() => onComplete(accuracy, accuracy, `${Math.floor(timeTakenMs/1000)}s`, finalCorrect, maxRounds - finalCorrect), 1000);
    }
  };
    ...
    <div className="card" style={{ textAlign: 'center', padding: '3rem' }}>
      ...
      {showNumber ? (
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1.5rem' }}>
          ...
          <input type="number" value={input} onChange={e => setInput(e.target.value)} required autoFocus style={{ fontSize: '2rem', padding: '1rem', width: '250px', textAlign: 'center', borderRadius: '8px', border: '2px solid var(--secondary-color)' }} />
...
function WordRecall({ onComplete, soundEnabled, triggerFeedback }) {
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => onComplete(accuracy, accuracy, `${Math.floor(timeTakenMs/1000)}s`, correctCount, 3 - correctCount), 1000);
  };
...
function AttentionTest({ onComplete, soundEnabled, triggerFeedback }) {
  ...
  useEffect(() => {
    if (gameState === 'finished') {
      ...
      setTimeout(() => onComplete(Math.max(0, accuracy), accuracy, `${Math.floor(timeTakenMs/1000)}s`, correctTaps, incorrectTaps + missed), 1000);
    }
...
export default function GameView() {
  ...
  const handleGameComplete = (score, accuracy, timeTaken, correct, incorrect) => {
    ...
    navigate(`/games/${gameId}/results`, { state: { score, accuracy, timeTaken, correct, incorrect } });
  };
